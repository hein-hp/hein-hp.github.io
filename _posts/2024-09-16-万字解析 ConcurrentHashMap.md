...
我们知道 HashMap 是非线程安全的，为了保证并发场景下的线程安全，我们可以使用一些具备线程安全性的集合，比如 Hashtable、Collections.synchronizedMap 以及 ConcurrentHashMap。

但是 Hashtable 和 Collections.synchronizedMap，都是基于 synchronized 实现的线程安全，在线程竞争激烈的情况下，效率都比较低，这时候就可以考虑使用锁粒度更小的 ConcurrentHashMap。

同时，在 ConcurrentHashMap 中，有很多共同点，比如在发生哈希冲突时使用拉链法解决哈希冲突、链表和红黑树转换的阈值选择等，我们这里就不多解释。
...
![image-20240916215007465](/img/image-20240916215007465.png)
...
spread() 方法通过将哈希值的高位与低位异或（XOR）来分散高位的影响，使得高位的变化也能够影响到索引的选择。
...
private final void treeifyBin(Node<K,V>[] tab, int index) {
    ...
    if (tab != null) {
        ...
        // 由于红黑树可能会发生旋转从而导致头节点发生变化，如果仍然使用 TreeNode 加锁存在问题
...
+ 如果 sizeCtl 的值为 -4，表示当前有两个线程正在进行扩容。
...
// 到这里就是正在进行扩容
    ...
    if (sc < 0) {
        ...
        if ((sc >>> RESIZE_STAMP_SHIFT) != rs || sc == rs + 1 ||
            ...
            transferIndex <= 0)
            break;
        // 表示当前线程可以帮助扩容，CAS 将 sizeCtl + 1 + 1
        if (U.compareAndSwapInt(this, SIZECTL, sc, sc + 1))
            transfer(tab, nt);
    }
...
![image-20240916215042417](/img/image-20240916215042417.png)
