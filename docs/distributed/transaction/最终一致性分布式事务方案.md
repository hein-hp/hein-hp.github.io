...
这篇文章就来分享一下这个过程，顺便也整理一些常见的保证最终一致性的分布式事务解决方案。
...
在业界中，保证最终一致性的分布式事务方案还有一种，叫最大努力通知，它和本地消息表、事务消息一样，都是基于异步消息，而所谓最大努力通知，就是尽最大努力，但是不保证能够百分百能够通知到。

在消息生产、传递、消费的过程中，如果出现了网络或者消息队列故障，就可能导致消息丢失，一旦发生这种情况，最大努力通知模式会再次进行尝试，但是这个尝试的次数一定是有限的，所以我们说他不是百分百的。

最大努力通知适用于那些对最终一致性要求没那么严格的场景，比如我们在投简历的时候，投递成功后公司会发邮箱给你，这里发邮箱即使重试几次后还是失败了其实也没有关系，并不影响什么。

另外就是一些必须保证最终一致性，但是我们可以容忍达到这个一致性的时间较长， 那么就可以通过对账机制，通过人工介入来手动保证数据一致，总而言之，最终一定会保证业务的最终一致性。

最后，最大努力通知和本地消息表方案都需要消费端严格保证消费幂等性，因为这两种方案都可能出现重复投递的情况。

## 总结

总的来说，本地消息表、事务消息和最大努力通知适合于不要求强一致，但必须保证最终一致性的场景。

最大努力通知是最简单，也是对系统性能影响最小的，对于一般的场景我们都可以使用这种方式。

然后，事务消息和本地消息表两个方案是可以互相替换的，用了事务消息的地方都可以换成本地消息表，但实际上，如果 MQ 支持事务消息，才可以考虑这个方案，如果公司使用的 MQ 不支持事务消息，就可以考虑本地消息表。
