...
2PC（Two-Phase Commit） 即两阶段提交，是基于 XA 规范的分布式事务协议，所以我们下面先了解一下 XA 规范，而 3PC 则是 2PC 扩展出来的协议。
...
+ AP：Application 应用程序，它定义了事务的边界，负责调用 TM 开始、提交或回滚一个事务。
+ TM：Transaction Manager 事务管理器，它是负责协调和控制整个事务流程的组件，它可以管理一个或多个分支事务，确保所有参与的 RM 能够正确地执行事务操作。
+ RM：Resource Manager 资源管理器，它是具体管理数据存储的组件，例如具体的数据库系统 MySQL。RM 负责执行由 TM 指示的具体操作，比如开启、准备提交、实际提交或回滚事务。

在传统的 DTP 模型中，AP、TM 和 RM 的交互如下，下半部分算是 ShardingSphere 对 DTP 模型的一种细分。
