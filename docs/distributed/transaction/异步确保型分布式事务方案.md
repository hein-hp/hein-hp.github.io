...
这仅仅只是我们在代码结构层面做出的最优的考虑，实际上还存在一些问题，比如在本地事务提交后发送消息前服务故障宕机，或者是本地事务提交，但是发送消息超时，或者更常见的由于 OS PageCache 繁忙导致 MQ 出现 system busy、broker busy 问题，导致消息丢失，这些情况都会造成发送消息和本地事务无法保证原子性。
