...
比如，一条库存记录，处在补偿型事务处理过程中，由 100 扣减为 50，此时，仓库管理员连接数据库，查询统计库存，可以查询出此时的 50，之后，事务因为异常回滚，库存会被补偿回滚为 100，显然，仓库管理员查询统计到的 50 就是脏数据。
...
比如，上面提到的库存更新场景，XA 事务处理过程中，中间态数据库存 50 由数据库本身保证，是不会仓库管理员的查询统计看到的（当然隔离级别需要读已提交以上），就像下面这样：
...
在 XA 模式中，数据在整个事务处理过程结束前，都需要被锁定，读写都按隔离级别的定义约束起来。
...
这是 XA 协议的核心痛点，也是 Seata 引入 XA 模式要重点解决的问题，那么解决的基本思路是两个方面：
...
XA 模式性能的损耗主要来自两个方面：一是事务协调的过程增加了单个事务的 RT，另一方面是并发事务数据的锁冲突降低了系统吞吐量。
...
其次，并发事务之间存在数据热点，产生锁冲突的情况在 AT 模式下（默认使用全局锁）同样存在。
